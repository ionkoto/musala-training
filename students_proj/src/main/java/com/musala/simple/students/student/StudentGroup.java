 ...
 * The {@link Student} objects are stored in a Map. It allows the user
 * to add entries in a way, that each student's Id matches the object's 
 * index in the Map.
 * 
...
public class StudentGroup {
	 ...
	 * {@link Student} objects. An attempt to add a student with 
	 * existing ID returns a message to the user and the operation
	 * is omitted.
	 *
	...
	public void fillStudentGroup (Student[] studentsArr) {
		for (Student student : studentsArr) {
			if (this.students.containsKey(student.getId())) {
				System.err.printf("The student %s with id %d can not be added. A student with the same Id already exists.\n", 
	 ...
	 * Map of Student objects and returns it to the user.
	 *
	 ...
	 * Student object. If the Students map does not contain the provided id
	 * as a key the method throws an Exception.
	 *
